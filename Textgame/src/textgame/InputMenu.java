...
public class InputMenu {
	...
	public void tutorial() throws IOException {
		...
		while(tutorial_m.M_HP > 0) {
			...
			if("1".equals(input)) {
				tutorial_m.M_HP = tutorial_m.M_HP- c.HOUR_DMG;
				System.out.println(tutorial_m.M_name+"에게 "+c.HOUR_DMG+"의 데미지를 주었습니다.");
	...
	public void town() throws IOException {
		...
		while(true) {
			...
			if("1".equals(input)) {
				...
				System.out.println("수련장으로 향합니다.");
				menu.monsterSelect();
				String monster = sc.next();
				if("1".equals(monster)) {
					System.out.println("전기 기사(1h)를 만났습니다.");
					Monster m = new Monster("전기 기사(1h)");
					m.battle(c, m);
				}else if("2".equals(monster)) {
					System.out.println("전기 기사(2h)를 만났습니다.");
					Monster m = new Monster("전기 기사(2h)");
					m.battle(c, m);
				}else if("3".equals(monster)) {
					System.out.println("전기 기사(3h)를 만났습니다.");
					Monster m = new Monster("전기 기사(3h)를");
					m.battle(c, m);
				...
				else if("6".equals(monster)) {
					System.out.println("서큐버스을 만났습니다.");
					Monster m = new Monster("서큐버스");
					m.battle(c, m);
				}
				else if("7".equals(monster)) {
					System.out.println("서큐버스을 만났습니다.");
					Monster m = new Monster("서큐버스");
					m.battle(c, m);
				}
